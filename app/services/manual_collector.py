...
class ManualCollector:
    ...
    def collect_wallet(self, wallet_id: int) -> ChainCollectionResult:
        rows = list(
            self.db.scalars(
                select(ManualBalance).where(ManualBalance.wallet_id == wallet_id).order_by(ManualBalance.id)
            )
        )
        ...
        for row in rows:
            ...
            source = "manual"
            if price is None:
                price, source = self.price_service.get_usd_price(row.symbol)
            if price is None:
                value = Decimal("0")
                source = None
                missing_prices.append(row.symbol)
            else:
                value = row.value_usd if row.value_usd is not None else row.amount * price
            total += value
            balances.append(
                AssetBalance(
